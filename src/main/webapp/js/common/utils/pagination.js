...
let egovPaginationBtn = {
    ...
    getPaginationTag : function(paginationInfo, callbackFunc) {
        ...
        if (paginationInfo.totalPageCount > paginationInfo.pageSize) {
            if (paginationInfo.firstPageNoOnPageList > paginationInfo.pageSize) {
                temp += '<button title="맨 처음 페이지" onclick="' + callbackFunc + '(1);return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_agoprev.png"></button>';
                temp += '<button title="이전 페이지" onclick="' + callbackFunc + '(' + (paginationInfo.firstPageNoOnPageList - 1) + ');return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_prev.png"></button>';
            } else {
                temp += '<button title="맨 처음 페이지" onclick="' + callbackFunc + '(1);return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_agoprev.png"></button>';
                temp += '<button title="이전 페이지" onclick="' + callbackFunc + '(1);return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_prev.png"></button>';
            }
        }

        for (let i = paginationInfo.firstPageNoOnPageList; i <= paginationInfo.lastPageNoOnPageList; i++) {
            if (i === paginationInfo.currentPageNo) {
                temp += '<button title="선택된 페이지" class="on" onclick="' + callbackFunc + '(' + i + ')">' + i + '</button>';
            } else {
                temp += '<button onclick="' + callbackFunc + '(' + i + ')">' + i + '</button>';
            }
        }

        if (paginationInfo.totalPageCount > paginationInfo.pageSize) {
            if (paginationInfo.lastPageNoOnPageList < paginationInfo.totalPageCount) {
                temp += '<button title="다음 페이지" onclick="' + callbackFunc + '(' + (paginationInfo.firstPageNoOnPageList + paginationInfo.pageSize) + ');return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_next.png"></button>';
                temp += '<button title="맨끝 페이지" onclick="' + callbackFunc + '(' + (paginationInfo.totalPageCount) + ');return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_agonext.png"></button>';
            } else {
                temp += '<button title="다음 페이지" onclick="' + callbackFunc + '(' + (paginationInfo.totalPageCount) + ');return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_next.png"></button>';
                temp += '<button title="맨끝 페이지" onclick="' + callbackFunc + '(' + (paginationInfo.totalPageCount) + ');return false;" class="btn_arrow"><img src="' + contextPath + '/images/ico_agonext.png"></button>';
            }
        }
        return temp;
