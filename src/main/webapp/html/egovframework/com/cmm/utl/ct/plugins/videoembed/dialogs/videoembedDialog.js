...
CKEDITOR.dialog.add('videoembedDialog', function (editor) {
    return {
        ...
        onOk: function () {
            ...
            if (url.length > 1) {
                var iframe = new CKEDITOR.dom.element.createFromHtml('<div class="video_area"><iframe frameborder="0" width="900" height="480" src="' + url + '" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe><div>※ 영상출처: ' + url + '</div></div>');
                div_container.append(iframe);
                
                //var urlMsg = new CKEDITOR.dom.element.createFromHtml('<div>※ 영상출처: ' + url + '</div>')
                //div_container.append(urlMsg);
