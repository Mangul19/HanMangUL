...
public class MaskingUtils {
    ...
    public static String getTelnoMask(String telno) {
        ...
        /*
         * 전화번호 2번째 자리는 뒤에 2자 **, ex) 1234 -> 12** 전화번호 3번째 자리는 첫자만 *, ex) 1234 -> *234
         */
    ...
    public static String getAddrMask(String addr) {
        ...
        for (int i = 0; i < addrStrArr.length; i++) {
            ...
            if (addrStrArr[i].matches(regex)) {
                if (matchCnt == 0 || addrStrArr[i].substring(addrStrArr[i].length() - 1).equals("로") || addrStrArr[i].substring(addrStrArr[i].length() - 1).equals("길") || addrStrArr[i].substring(addrStrArr[i].length() - 1).equals("동")
